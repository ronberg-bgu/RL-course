...
class TabularSARSA:
    ...
    def train(self, total_training_steps=500_000, gamma=0.99, alpha=0.1,
        ...
        # Calculate initial epsilon (for step 0)
        ...
        for step in range(total_training_steps):
            ...
            # Epsilon for the action selection of the next step (step + 1)
            next_step = step + 1
            epsilon = epsilon_start - (next_step / epsilon_decay_steps) * (epsilon_start - epsilon_end)
            ...
            # SARSA TD update
            td_target = reward + gamma * self.q_table[next_state, next_action] * (not terminated)
            ...
            if terminated or truncated:
                ...
                # Select action for the new episode using the current epsilon
                if np.random.rand() < epsilon:
                    action = self.env.action_space.sample()
                else:
                    action = int(np.argmax(self.q_table[state]))
