...
def generate_domain(domain_path):
  ...
  (:action move
    ...
    :precondition (and (agent-at ?a ?from) (adj ?from ?to ?d) (clear ?to))
    :effect (and (agent-at ?a ?to) (not (agent-at ?a ?from)))
  )

  (:action push-small
    :parameters (?a - agent ?from - location ?boxloc - location ?toloc - location ?d - direction ?b - box)
    :precondition (and (agent-at ?a ?from) (adj ?from ?boxloc ?d) (box-at ?b ?boxloc) (adj ?boxloc ?toloc ?d) (clear ?toloc))
    :effect (and (agent-at ?a ?boxloc) (not (agent-at ?a ?from)) (clear ?boxloc) (box-at ?b ?toloc) (not (box-at ?b ?boxloc)) (not (clear ?toloc)))
  )

  (:action push-heavy
    :parameters (?a1 - agent ?a2 - agent ?from - location ?boxloc - location ?toloc - location ?d - direction ?h - heavybox)
    :precondition (and
        (not (= ?a1 ?a2))
        (agent-at ?a1 ?from)
        (agent-at ?a2 ?from)
...
def generate_problem(env, problem_path):
    ...
    # ── Clear set: locations not occupied by a box (agents may co-locate) ──
